...
preplot_phi <- function(x, start = 1, end = NA, pll = TRUE, ...) {
  ...
  if (pll) {
    ...
    step1 <- lapply(data.frame(t(lp)), function(xxx){
      coda::mcmc(as.matrix(xxx), start, end, thin = 1) ## thin must be 1 for coda to work
    })
...
##' @param chains numerical indexs indicating which chains to plot.
##' @param pll a boolean switch for plotting parameter values or posterior log_likelihoods
##' @param ll a boolean switch for plotting parameter values or log_likelihoods
##' @export
autocorr <- function(x, start = 1, end = NA, nLags = 50, subchain = FALSE,
                    nsubchain = 3, chains = NA, pll = FALSE, ll = FALSE, ...)
{
  if (x$n.chains == 1) stop ("MCMC needs multiple chains to check convergence")
  if (is.null(x$theta)) stop("Use hyper mcmc_list")
  if (is.na(end)) end <- x$nmc
  ...
  if (end < nLags)
  {
    warning(sprintf("nLags=%d is larger than number of iterations, computing until max possible lag %d", nLags, end))
    nLags <- end
  }

  D <- ConvertChains(x, start, end, pll, ll)

  if (subchain) {
    if (missing(nsubchain)) stop("Please supply nsubchain")
    if (any(is.na(chains))) chains <- sample(unique(D$Chain), nsubchain)
    cat("Plot chains: ", chains, "\n")
    D <- D[ D$Chain %in% chains, ]
  }

  D_ac <- D[, .(Lag = ac(value, nLags)[[1]],
