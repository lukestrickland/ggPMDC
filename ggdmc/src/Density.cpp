...
// [[Rcpp::export]]
arma::cube trial_loglik(List samples, unsigned int thin_pointwise)
{
  ...
  arma::cube theta    = samples["theta"]; // npar x nchain x nmc
  ...
  for (size_t k=0; k<nchain; k++)
  {
    ...
    { // sample at certain regular iterations to calculate likelihoods
      j    = nmc_thin[i];
      tmp  = likelihood(theta.slice(j).col(k), data);
